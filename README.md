...
A complete, working Minit game on [Phaser 4](https://phaser.io) — scenes and
Arcade physics out of the box.
...
npm run package    # build, verify, and write dist/minit-template-phaser.zip
```

Upload `dist/minit-template-phaser.zip` at [console.minit.games](https://console.minit.games).

**Size.** ~2.07 MB built (623 KB gzipped). Phaser is the largest of these engines; well under the 5 MB ceiling, well over the "under 1 MB" guidance.

## Layout

```
index.html            page shell + the Minit audio repair (read the comment)
src/main.js           SDK lifecycle, scoring, HUD, the round clock
src/scene.js          the Phaser.Scene: textures, Arcade bodies, layout
...
**The physics is Phaser's, not hand-written beside it.** The ball is an Arcade
body with gravity and bounce, the floor and the walls *are* the world bounds,
and the bounce is a `worldbounds` event the scene listens for rather than a
position test in `update`. The ceiling matters as much as the floor: a tap
*sets* upward velocity, so a rally climbs, and without a bound the ball leaves
the top of the screen exactly when the player is doing well — where it cannot
be tapped and the rally dies waiting for it to fall back.

**Do not accumulate Phaser's `delta` for a game clock.** Phaser smooths delta
across frames, and summing a smoothed value drifts: a 30 second round measured
**31.9 s** that way. The clock differences Phaser's absolute `time` instead —
which is still Phaser's own clock, so it pauses when the page is hidden — and
`fps.smoothStep` is off so the two notions of elapsed time agree.
